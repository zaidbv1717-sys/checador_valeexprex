(function(){
  ...
  function renderClock(){
    ...
    root.innerHTML = `
      <div class="wrap">
        <div class="topbar"><div class="brand">Reloj checador</div><button class="admin-link" id="btn-goadmin">Admin</button></div>
        <div class="card">
  ...
  function renderAdminLogin(){
    root.innerHTML = `
      <div class="wrap">
        <div class="topbar"><div class="brand">Reloj checador</div></div>
  ...
  function renderAdmin(){
    ...
    if(state.adminTab==='registros'){
      ...
      content = `
        ...
        <div class="row">
          <div style="flex:1;"><label style="font-size:11px;color:var(--muted);">Entrada esperada</label><input type="time" id="newschedin" value="09:00" /></div>
          <div style="flex:1;"><label style="font-size:11px;color:var(--muted);">Salida esperada</label><input type="time" id="newschedout" value="18:00" /></div>
        </div>
        <div class="row"><button class="btn" id="btn-addemp" style="flex:1;">Agregar empleado</button></div>
        <div style="margin-top:6px;">
          ${state.employees.length? state.employees.map(e=>`
            <div class="emp-item">
              <span>${e.name} <span class="pin">(${e.sched_in}–${e.sched_out})</span></span>
    ...
    if(state.adminTab==='empleados'){
      document.getElementById('btn-addemp').onclick = async ()=>{
        ...
        const schedIn = document.getElementById('newschedin').value || '09:00';
        const schedOut = document.getElementById('newschedout').value || '18:00';
        const r = await api('/api/admin/employees', {method:'POST', body: JSON.stringify({name, pin, schedIn, schedOut})});
