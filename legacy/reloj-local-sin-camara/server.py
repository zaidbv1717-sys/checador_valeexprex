...
def compute_report_rows(period, anchor_date_str, emp_filter):
    ...
    lunch_minutes = int(get_config().get('lunch_minutes', '90') or 90)
...
def build_csv(rows, period, date_str, emp_filter):
    ...
    w.writerow([])
    w.writerow(['Empleado', 'Fecha', 'Entrada', 'Salida a comer', 'Regreso de comer', 'Salida', 'Horas trabajadas', 'Estado'])
    ...
    for r in sorted_rows:
        ...
        fecha = datetime.strptime(r['date'], '%Y-%m-%d').strftime('%d/%m/%Y')
        estado_parts = []
        if r['missing']:
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path == '/api/admin/employees':
            ...
            sched_in = body.get('schedIn', '09:00') or '09:00'
            sched_out = body.get('schedOut', '18:00') or '18:00'
            ...
            conn.execute("INSERT INTO employees (id, name, pin, sched_in, sched_out) VALUES (?,?,?,?,?)",
                         (uid(), name, pin, sched_in, sched_out))
            conn.commit()
            conn.close()
            return self.send_json({'ok': True})
